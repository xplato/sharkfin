...
final class DirectoryWatcherService {
    ...
    func restartIfNeeded() {
        let enabled = UserDefaults.standard.object(forKey: "watchDirectories") as? Bool ?? true
        guard enabled,
        ...
        let watchedDirs = store.directories.filter { $0.enabled && $0.bookmark != nil }
        ...
        let callback: FSEventStreamCallback = {
            ...
            for i in 0..<numEvents {
                if let path = unsafeBitCast(
                    CFArrayGetValueAtIndex(cfArray, i), to: CFString?.self
                ) as String? {
                    paths.append(path)
                }
            }
    ...
    fileprivate func handleFSEvents(paths: [String]) {
        ...
        for dirId in affectedIds {
            ...
            debounceTasks[dirId] = Task { [weak self, weak indexingService, weak directoryStore] in
                do {
                    try await Task.sleep(for: self?.debounceInterval ?? .seconds(3))
                } catch { return } // cancelled

                guard let indexingService, let directoryStore else { return }
                guard let dir = directoryStore.directories.first(where: { $0.id == dirId }) else { return }
