  ...
  var body: some View {
    HStack(alignment: .center, spacing: 10) {
        ...
        isOn: Binding(
          ...
          set: { newValue in
            ...
            if !newValue && !suppressDisableWarning {
              ...
              try? store.database.updateDirectoryEnabled(
                id: id,
                enabled: newValue
              )
            }
      ...
      .confirmationDialog(
        ...
        Text(
          "This directory will be removed from Sharkfin. New indexing will not be performed, and existing analysis, thumbnails, and references will be deleted. The contents of the directory will remain unchanged."
        )
      }
    }
    .padding(.vertical, 6)
    ...
    .alert(
      ...
      Text(
        "Search results from this directory will be hidden while it is disabled."
      )
    }
